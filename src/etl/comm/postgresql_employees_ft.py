...
print(df_sociedad)
print(f"Distinct entries: {len(df_sociedad.groupby(['id']))}")"""
...
curr = m_dbCon.cursor()
curr.execute('truncate table "people"."PPL_EMPLOYEES_FT_NEW"')
curr.close()
m_dbCon.commit()
...
"""postgresql_client = PostgreSQLClient(**credentials['people_write'], lazy_initialization = True)
df_total.to_csv(os.path.join(os.environ['USERPROFILE'], 'output.csv'))
credentials_postgre = credentials['people_write']
import psycopg2
m_dbCon = psycopg2.connect(user=credentials_postgre['username'], password=credentials_postgre['password'], host=credentials_postgre['host'] 
,database=credentials_postgre['database'])
